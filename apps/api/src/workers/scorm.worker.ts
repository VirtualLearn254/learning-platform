...
export function startScormWorker() {
  return new Worker<JobData>(QueueNames.ScormBuild, async (job) => {
    ...
    try {
      ...
      // Branching (LP-18): a wrong answer can detour into a rendered
      // remediation clip (an isAlt beat referenced by quiz.branches) and then
      // resume the lesson. Clips ship INSIDE the zip (alt/<beatKey>.mp4) so
      // LMS-hosted lessons stay fully self-contained; the hosted preview
      // streams the same clips from S3 instead of duplicating bytes.
      type QuizBranchSpec = { onOptionId: string; altBeatKey: string; returnToBeatKey: string };
      const altBeatByKey = new Map(beats.filter((b) => b.isAlt && b.mp4Key).map((b) => [b.beatKey, b]));
      const altClips: Array<{ path: string; data: Buffer }> = [];
      const altPreviewSrc: Array<[zipPath: string, publicUrl: string]> = [];
      for (const cue of quizzes) {
        const beat = mainBeats.find((b) => b.beatKey === cue.beatKey);
        const branches = ((beat?.quiz as { branches?: QuizBranchSpec[] } | null)?.branches ?? [])
          .filter((br) => altBeatByKey.has(br.altBeatKey));
        const first = branches[0]; // one remediation clip per question
        if (!first) continue;
        const alt = altBeatByKey.get(first.altBeatKey)!;
        const zipPath = `alt/${first.altBeatKey}.mp4`;
        if (!altClips.some((cl) => cl.path === zipPath)) {
          const clip = await s3.getObject(alt.mp4Key!);
          altClips.push({ path: zipPath, data: Buffer.from(clip.body) });
          altPreviewSrc.push([zipPath, `/api/files/${alt.mp4Key}`]);
        }
        const resumeAt = beatStart[first.returnToBeatKey];
        cue.branch = { file: zipPath, ...(resumeAt != null ? { resumeAtSec: resumeAt } : {}) };
      }
      if (altClips.length > 0) await note(`${altClips.length} branch clip(s) packaged`);

      // Lesson notes (LP-19): the designer-AI reference PDF drafted from the
      // SOURCE material ships inside the zip so the LMS delivers it as lesson
      // notes. Missing notes never block a publish — we queue generation so
      // the next re-publish picks them up.
      let notesPdf: Buffer | undefined;
      if (lesson.notesPdfKey) {
        try {
          const obj = await s3.getObject(lesson.notesPdfKey);
          notesPdf = Buffer.from(obj.body);
          await note(`lesson notes packaged (${(notesPdf.length / 1024).toFixed(0)} KB)`);
        } catch (nErr) {
          console.warn(`[scorm:${jobId.slice(0, 8)}] notes download failed (publish continues):`, nErr instanceof Error ? nErr.message : nErr);
        }
      } else {
        await queues.notes.add("notes-on-publish", { lessonId }).catch(() => {});
        console.log(`[scorm:${jobId.slice(0, 8)}] no lesson notes yet — generation queued; re-publish to include them`);
      }
      ...
      let previewHtml = built.playerHtml.replace(
        'src="master.mp4"',
        `src="/api/files/${lesson.masterMp4Key}"`,
      );
      // Branch cues point at zip-relative paths; the preview streams the same
      // clips from the existing S3 objects instead.
      for (const [zipPath, publicUrl] of altPreviewSrc) previewHtml = previewHtml.split(zipPath).join(publicUrl);
      await Promise.all([
      ...
      try {
        ...
        // The narration-derived summary.pdf is superseded by the source-
        // material lesson notes (LP-19) once those exist.
        await Promise.all([
          s3.putObject(`lessons/${lessonId}/content.pdf`, pdfs.content, { contentType: "application/pdf" }),
          ...(notesPdf ? [] : [s3.putObject(`lessons/${lessonId}/summary.pdf`, pdfs.summary, { contentType: "application/pdf" })]),
        ]);
        await note(`PDFs uploaded (content ${(pdfs.content.length / 1024).toFixed(0)} KB${notesPdf ? "" : ` + summary ${(pdfs.summary.length / 1024).toFixed(0)} KB`})`);
      } catch (pdfErr) {
      ...
      // Main beats + any alt beats that actually shipped in the package.
      const shippedAltKeys = new Set(altClips.map((cl) => cl.path.slice("alt/".length, -".mp4".length)));
      for (const beat of beats.filter((b) => !b.isAlt || shippedAltKeys.has(b.beatKey))) {
