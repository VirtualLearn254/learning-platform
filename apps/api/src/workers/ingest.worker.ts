...
const BeatSchema = z.object({
  ...
  beatType: BeatType,
  /** Short outline of what this beat will cover (the author worker expands
   *  this into the full narration script later). */
  script: z.string().min(10).max(400),
});
...
const SYSTEM_PROMPT = `You convert educational documents into a COURSE OUTLINE (structure only — narration scripts come later, do NOT write full scripts).
              ...
              "beats": [
                {
                  ...
                  "script":   string (ONE SHORT SENTENCE: 10-30 words describing what this beat covers — NOT narration)
                }
              ]
            }
          ]
...
SIZING (strict, to stay under the output token budget):
- 1-4 sections (mirror the document's major themes)
- 1-3 modules per section
- 2-4 lessons per module
- 4-7 beats per lesson, following the arc: hook (1) -> concept (1-3) -> example (1) -> check (1) -> recap (1)
- CHECK-BEAT PLACEMENT: place the check immediately after the concept/example it tests (never after the recap). For lessons with 2+ distinct concepts, one check per major concept is allowed (max 2).
- CHECK-BEAT TYPE (note the suggested interaction in the outline — the author picks the final type from this catalog): computed answers -> fill_in or estimate; procedures/sequences -> ordering; classification -> sort_into or this_or_that; definitions/recall -> true_false, flashcard, or memory_pairs; concept-rule pairs -> match; principle statements -> word_bank; judgment/soft-skills -> scenario; select-all diagnostics -> multi_select; term identification -> guess_concept; vocabulary-dense lessons -> word_search (sparingly); reflection -> likert.
- CHECK-BEAT VARIETY (you can see the WHOLE course here — this is the only place to guarantee it): give EVERY check a DISTINCT interaction type. There are 16 types and rarely more than ~8 checks, so all-distinct is almost always achievable — aim for it. Absolute rules: never the same type twice in a row, never twice within one module. Only repeat a type if the course has more checks than usable types. State the chosen interaction explicitly in each check's outline (e.g. "This-or-that check: ..."). Step Bloom level up as the course progresses (recall/understand early, apply/analyze later).

CONTENT:
- The "script" field is a short outline, ONE sentence describing the beat's intent. The full narration script is generated by a later step.
- Use the document's own vocabulary in titles and outlines.
- Write math in plain text ("x squared", "x^2"), never unicode superscripts.
- Beat keys are slug-style: "linear_eq_definition", "slope_intercept_form".
  ...
  return new Worker<JobData>(QueueNames.Ingest, async (job) => {
    ...
    try {
      ...
      const ai = await client.chat("ingest", {
        messages: [
          { role: "system", content: SYSTEM_PROMPT + await getRulesBlock("ingest") },
